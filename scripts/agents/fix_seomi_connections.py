"""Fix connections for Seomi's 4 new Google tools — point to correct node name."""
...
correct_target = "Seomi - SEO Specialist"
tools_to_fix = [
...
for tool in tools_to_fix:
    conns[tool] = {"ai_tool": [[{"node": correct_target, "type": "ai_tool", "index": 0}]]}
    print(f"Fixed: {tool} -> {correct_target}")
...
with open('C:/Users/sales/.claude/seomi_fix_payload.json', 'w', encoding='utf-8') as f:
    json.dump(api_payload, f)

print(f"\nPushing {len(data['nodes'])} nodes...")
os.system(f'curl -s -X PUT "{BASE}/workflows/JAYrzGWR8A0tCBzB" -H "X-N8N-API-KEY: {API_KEY}" -H "Content-Type: application/json" -d @"C:/Users/sales/.claude/seomi_fix_payload.json" -o "C:/Users/sales/.claude/seomi_fix_response.json"')

with open('C:/Users/sales/.claude/seomi_fix_response.json', encoding='utf-8') as f:
    resp = json.load(f)
