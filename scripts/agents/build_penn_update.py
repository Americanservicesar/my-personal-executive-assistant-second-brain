...
# ============================================================
# 1. PENN SYSTEM MESSAGE + DESCRIPTION
# ============================================================
...
## REFERENCE FILES
- Sales PDF: Google Drive SALES folder
- Logo folder: Google Drive MARKETING folder
- Document Header Template: use for all formal proposals
...
for node in data['nodes']:
    if node['name'] == 'Penn - Writing Agent':
        ...
        print(f"Updated Penn system message: {len(PENN_SYSTEM_MESSAGE)} chars")
        print(f"Updated Penn description: {len(PENN_DESCRIPTION)} chars")
        break

# ============================================================
# 2. ADD NEW TOOL NODES FOR PENN
# ============================================================

# Penn currently has: Penn Claude Model, Gmail Tool - Penn (no web search node found connected)
# Plan says add: SerpApi, Google Drive, Google Docs, Google Sheets, Slack
# Penn's position is [912, 224], tools below at ~432+
...
# Google Docs - Penn (matching Google Docs - Vizzy: typeVersion 2, folderId + title)
new_nodes.append({
    "parameters": {
        "folderId": "root",
...
for node in new_nodes:
    conns[node['name']] = {
        "ai_tool": [[{"node": penn_agent, "type": "ai_tool", "index": 0}]]
    }

print(f"\nConnected {len(new_nodes)} new tools to Penn")
...
payload_path = r'C:\Users\sales\.claude\penn_payload.json'
resp_path = r'C:\Users\sales\.claude\penn_response.json'
