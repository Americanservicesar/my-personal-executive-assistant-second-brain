...
- **ALWAYS use gdrive.py (gcloud CLI) for ALL Google Drive operations** — rename, move, create, list, search, upload, info. This rule applies to ALL agents, ALL sessions, ALL platforms (Claude, n8n, any automation). Google Workspace MCP `renameFile`/`moveFile` fails with "File not found" due to ownership/permissions issues even on files you own. gdrive.py uses gcloud account auth which has full access to all Shared Drives. **NEVER use Google Workspace MCP for Drive file/folder operations — always use gdrive.py.**
...
|-------|--------|----|----------|
| MARKETING | Meta Ads | `1pnkZR_SRROHCnqBM5bmbLSRE95k4Y6dD` | Audience Inventory, Operational Game Plan, Launch Checklist, Audience Build Guide |
