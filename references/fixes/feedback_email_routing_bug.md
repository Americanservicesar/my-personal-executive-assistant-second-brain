...
## Notification Sender Blocklist (Filter Internal Senders node — mqSWSLhNl3Qy0Nyy)

These senders are blocked in the Gmail Monitor → Milli workflow as of 2026-04-28:
- `@americanservicesar.com` — internal loop prevention
- `sonsfamily2012@gmail.com` — personal account
- `@roofsnap.com` — order completion notifications (orders@roofsnap.com) — Milli was replying to these as if they were customer emails, causing outbound emails to sales@ and to the RoofSnap report customer name
- `@braintreegateway.com` — payment receipts from Braintree/RoofSnap purchases

**Rule:** Any time a new automated notification service sends to sales@, add its domain to the Filter Internal Senders IF node in mqSWSLhNl3Qy0Nyy.
